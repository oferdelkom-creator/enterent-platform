...
export default function RequestPanel({
  ...
  if (sent) {
    return <p className="text-xs font-medium text-emerald-700">Request sent</p>;
  }
  ...
  return (
    <form action={handleSubmit} className="space-y-2 rounded-md border border-slate-200 bg-slate-50 p-3">
      ...
      <p className="text-xs font-medium text-slate-700">
        {type === "swap" ? "Propose a stay swap" : "Ask for emergency backup"}
      </p>
      ...
      {type === "swap" && ownListings.length > 0 && (
        ...
        >
          <option value="">Offer which of your listings?</option>
          {ownListings.map((l) => (
